  ...
  execute() {
    ...
    async.parallel([
      ...
      // console.log(results[1].objects.length)
        // optional callback
      let response = '';
      response = response.concat('Mark has had ' + JSON.parse(results[0]).objects[0].length + ' beers and')
      response = response.concat('Gary has had ' + JSON.parse(results[0]).objects[1].length + ' beers')
